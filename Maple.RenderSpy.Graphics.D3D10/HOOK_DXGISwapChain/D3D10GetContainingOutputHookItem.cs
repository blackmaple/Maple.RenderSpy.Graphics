...
{
    internal class D3D10GetContainingOutputHookItem : HookItem<D3D10GetContainingOutputHookItem, Ptr_Func_GetContainingOutput_15, Ptr_Func_GetContainingOutput_15>, IHookItemFactory<D3D10GetContainingOutputHookItem>
    {
        ...
        public static D3D10GetContainingOutputHookItem Create(IHookFactory hookFactory, IRenderSpyGraphicsFunctionsProvider functionsProvider)
        {
            if (!functionsProvider.TryGetGraphicsFunctions(MethodName, out var functionPtr))
            {
                return RenderSpyGraphicsException.Throw<D3D10GetContainingOutputHookItem>($"NOT FOUND {MethodName}");
            }
