...
namespace Maple.RenderSpy.Graphics.D3D9.HOOK_Direct3DDevice9
{
    internal class D3D9ClearHookItem : HookItem<D3D9ClearHookItem, Ptr_Func_Clear_43, Ptr_Func_Clear_43>, IHookItemFactory<D3D9ClearHookItem>
    {
        ...
        public Func<COM_PTR_IUNKNOWN<COM_INTERFACE_Direct3DDevice9>, uint, nint, uint, uint, float, uint, D3D9ClearHookItem, COM_HRESULT>? SyncCallback { get; set; }
        ...
        {
            delegate* unmanaged[Stdcall]<COM_PTR_IUNKNOWN<COM_INTERFACE_Direct3DDevice9>, uint, nint, uint, uint, float, uint, COM_HRESULT>
                _proc = &Hook_Clear;
            return new(_proc);
        }

        [UnmanagedCallersOnly(CallConvs = [typeof(CallConvStdcall)])]
        private static COM_HRESULT Hook_Clear(COM_PTR_IUNKNOWN<COM_INTERFACE_Direct3DDevice9> @this, uint Count, nint pRects, uint Flags, uint Color, float Z, uint Stencil)
        {
            if (D3D9ClearHookItem.TryGet(out var hookItem))
            {
                if (hookItem.SyncCallback is not null)
                {
                    return hookItem.SyncCallback.Invoke(@this, Count, pRects, Flags, Color, Z, Stencil, hookItem);
                }
